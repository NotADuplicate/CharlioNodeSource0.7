/// @description Draw on shop surface
if(room == room1) {
	xp = x;
	yp = y;
	if(clicked) {
	...
	//Draw stacks bought
	var spacing = 80/(maxStacks+1)
	spacingX = xp-40+spacing;
	var i = 0;
	while(i < maxStacks) {
		if(stacks > i) {
			draw_rectangle_color(spacingX-4,yp+35,spacingX+4, yp+43, c_green, c_green, c_green, c_green, false);
		}
