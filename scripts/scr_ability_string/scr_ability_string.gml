...
function scr_ability_string(ability){
	switch(ability) {
		case "Hook":
			return("Launches a hook which pulls players and the ball towards you. Enemy players are stunned while being hooked. Pulls you towards a wall if it hits one. Release the button to stop hooking");
		break;
		case "heal":
			return("Shoots a slow moving projectile which goes through walls and heals any player 150 health. Costs " + /*scr_ability_stats("heal","Ammo")*/ + "3 ammo.");
		break;
		case "dash":
			return("Triples your speed for 0.5 seconds. Costs " + /*scr_ability_stats("dash","Ammo")*/ + "1 ammo.");
		break;
		case "push":
			return("Create an object at your mouse which pushes all players and the ball away from it. The force is greater the closer to the object they are.");
		break;
		case "wall":
			return("Creates a wall where your mouse is. Can be used to block projectiles or players or the ball. Attacking the wall damages it");
		break;
		case "airstrike":
			return("Marks a visible line where the airstrike will land in 3 seconds dealing large damage and knockback. Good for area denial");
		break;
		case "reflect":
			return("Creates a reflect object directly in front of you which lasts 2 seconds and causes any projectile that hits it to go in the direction the reflect is pointing.");
		break;
		case "Stasis":
			return("Use to freeze a player or ball for 3 seconds. While stasised players cannot move, attack, or recieve damage. Any knockback recieved while stasised will act all at once when stasis is over.");
		break;
		case "Chainbreaker":
			return("Chain yourself to any nearby enemy or ball. If they are still in range of the chain after 3 seconds of being chained, they take 175 damage and lots of knockback. Particularly good at pushing the ball.");
		break;
		case "mine":
			return(spr_mine);
		break;
		case "flash":
			return("Instantly teleport a short distance. Can be used to jump over walls.");
		break;
		case "teleport":
			return("Place a marker anywhere on the map and teleport there in 1 second.");
		break;
		case "finisher":
			return("Shoot a fast moving projectile which goes through walls and kills targets who are below 35% health.");
		break;
		case "invisibility":
			return("Beome invisible to enemies for 4 seconds.");
		break;
		case "molotov":
			return("Throw a molotov which creates a fire when it lands, dealing 45 damage per second and applying fire to anyone who stands in it (excludes allies but not you).");
		break;
		case "ward":
			return(spr_ward);
		break;
		...
		break;
		case "Enrage":
			return("Use to enrage a nearby player including yourself. While enraged, the target moves 50% faster, shoots twice as fast, has half cool downs, but also takes increasing damage over time. Enrage goes away upon dying, going back to base, or standing still for 3 seconds.");
		break;
		case "Block":
		...
		break;
		case "Health Bomb":
			return("Throw a bomb which heals nearby allies by 60 health and damages enemies by 30 damage when it lands");
		break;
		case "Portal Gun":
		...
		break;
		case "Rocket":
			return("Shoot a rocket which deals 40 damage on impact and explodes");
		break;
		case "":
			return ""
		break;
