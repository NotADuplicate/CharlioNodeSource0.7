function scr_mafia_receive() {
	...
	switch(message_id) {
	    ...
	    case 2: 
			...
			ins = instance_create(0,0,mafia_game);
	        ins.alarm[3] = 1;
	    ...
	    break;
	    case 5: //die
			shot = buffer_read(buffer,buffer_u8);
			mafia_game.timer = 20;
			mafia_game.dying[shot] = 1;
			global.state = "Night";
			global.gun = 0;
			if(shot != 0)
				instance_destroy(global.players[shot])
			if(shot == obj_client.index)
				global.dead = true;
	    break;
	    case 6: //send ready
			num = buffer_read(buffer,buffer_u8);
	    ...
	    break;
	    case 7: //send actions
			abilityuser = buffer_read(buffer,buffer_u8);
			abilitytarget = buffer_read(buffer,buffer_u8);
			global.actionTarget[abilityuser] = abilitytarget;
			mafia_game.alarm[4] = 30;
	    break;
